...
def o2b_delta_rule_recurrent(
    ...
    # State computation always uses float32 for numerical stability
    # (prevents error accumulation in recursive updates)
    if W_t.dtype != torch.float32:
        W_t = W_t.float()
    if W_avg.dtype != torch.float32:
        W_avg = W_avg.float()
    ...
    for i in range(L):
        ...
        # State updates MUST use float32
        ...
        # Compute output: o = q @ W_avg (state is float32)
        o_t = q_t.float() @ W_avg
        outputs.append(o_t.to(output_dtype))

    o = torch.cat(outputs, dim=2).transpose(1, 2)  # (B, H, L, D) -> (B, L, H, D)

    # Return state in float32 for next iteration
...
def o2b_delta_rule(
    ...
    for i in range(num_chunks):
        ...
        W_avg_new = (coef1 * W_avg + coef2 * W_t +
                     (K_i.transpose(-2, -1) @ (U_i * w_i.view(1, actual_C, 1))))
        ...
        O_i = (QW_avg * a.view(1, actual_C, 1) +
